...
class SistemaPlanetarioActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if(intent.hasExtra("sistema_planetario")){
            val sistema_planetario2 = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
            ...
            CoroutineScope(Dispatchers.IO).launch {
                if (sistema_planetario2 != null) {
                    database.sistemaPlanetario().delete(sistema_planetario2)
